...
Splits are currently accounting-first payout plans, not settlement splits. A saved split belongs to one project and contains a settlement receiver plus payout receiver addresses with basis-point shares totaling 100%. `split-payment` and `revenue_split` intents verify the total USDC payment to the settlement receiver and attach `splitId`, `settlementReceiver`, and a JSON `splitPlan` metadata field containing each recipient and computed raw-USDC allocation. Inline SDK/dashboard split arrays are converted into the same plan shape. When the intent settles, ArcFlow issues the receipt with the split breakdown and no-auto-disbursement disclaimer, includes a structured `split` object in the signed webhook payload, and records `split.recorded`; it does not auto-payout funds in this MVP. Real settlement splits require an ArcFlowSplitter contract path: wallet approves ERC-20 USDC, calls `payAndSplit(...)`, the contract transfers exact amounts to recipients, emits a settlement event, and ArcFlow verifies that event plus recipient transfers.
